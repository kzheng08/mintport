...
There are four different hint styles — you can change the style by opening the block’s **Options menu** ![The Options menu icon in GitBook](../../.gitbook/assets/25_01_10_options_light.svg) and selecting the style you want. Each style uses a default icon, but you can customize the icon by clicking on it and choosing another one from [our icons set](../formatting/inline#icons).

Hint blocks support [inline content](../formatting/inline) and [formatting](../formatting/), as well some specific block types. To see which block types you can use in a hint, hit `/` on an empty line and check the [insert palette](./#inserting-a-new-content-block).
...
{/* gitbook: hint style="info" icon="books" %} */}
