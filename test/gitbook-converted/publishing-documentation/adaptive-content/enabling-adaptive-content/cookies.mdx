...
This method only works if your site is served under a [custom domain](../../custom-domain).
</Warning>
...
MethodUse-casesEase of setupSecurityFormatSigned cookie `gitbook-visitor-token`API test credentials, customer identificationRequire signing and a custom domain✅ Properties can only be defined by the backendJWTPublic cookie `gitbook-visitor-public`Feature flags, rolesEasy to set up❌ Visitor can override the propertiesJSON
