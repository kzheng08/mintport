...
description: "Add custom tools to GitBook Assistant to allow it to interact with your product, website, and more"
---
...
<Note>
In addition to custom tools you define in your Docs Embed configuration, the Assistant will always have access to any [MCP servers you define](../../mcp-servers-for-published-docs) in your site's AI settings.
</Note>
...
KeyDescription`name`Unique tool identifier.`description`Allows GitBook Assistant to know the context on when to use the tool being defined.`inputSchema`JSON schema defining the tool's input parameters. Can be accessed in the `input` argument in the `execute` call.`confirmation`An optional confirmation button that allows you to specify user interaction before firing the execute function. Format: `{ icon?: string, label: string }`.`execute`The async function that runs when the tool is called. Must return `{ output: any, summary: string }`. The `output` is provided to the AI to continue working with (not shown to the user). The `summary` is the visual summary shown in the user's chat window.
